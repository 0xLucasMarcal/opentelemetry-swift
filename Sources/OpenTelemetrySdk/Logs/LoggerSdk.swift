...
public final class LoggerSdk: OpenTelemetryApi.Logger, @unchecked Sendable {
  ...
  public func eventBuilder(name: String) -> OpenTelemetryApi.EventBuilder {
    guard let eventDomain else {
      // Events cannot be emitted from Logger without an event domain
      return DefaultLoggerProvider().loggerBuilder(instrumentationScopeName: "unused").setEventDomain("unused").setAttributes(["event.domain": AttributeValue.string("unused"), "event.name": AttributeValue.string(name)]).build().eventBuilder(name: "unused")
    }
